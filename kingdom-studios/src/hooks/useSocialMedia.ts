...
interface UseSocialMediaReturn {
  ...
  getAccountsForPlatform: (platformId: string) => SocialMediaPlatform[];
  addPlatformAccount: (platformId: string, authData: any) => Promise<boolean>;
  removePlatformAccount: (platformId: string, accountId: string) => Promise<boolean>;
  setActiveAccount: (platformId: string, accountId: string) => void;
  getActiveAccount: (platformId: string) => SocialMediaPlatform | undefined;
...
export const useSocialMedia = (): UseSocialMediaReturn => {
  ...
  const getAccountsForPlatform = useCallback((platformId: string) => {
    return socialMediaService.getAccountsForPlatform(platformId);
  }, [socialMediaService]);

  const addPlatformAccount = useCallback(async (platformId: string, authData: any) => {
    const result = await socialMediaService.addPlatformAccount(platformId, authData);
    setSupportedPlatforms(socialMediaService.getSupportedPlatforms());
    setConnectedPlatforms(socialMediaService.getConnectedPlatforms());
    return result;
  }, [socialMediaService]);

  const removePlatformAccount = useCallback(async (platformId: string, accountId: string) => {
    const result = await socialMediaService.removePlatformAccount(platformId, accountId);
    setSupportedPlatforms(socialMediaService.getSupportedPlatforms());
    setConnectedPlatforms(socialMediaService.getConnectedPlatforms());
    return result;
  }, [socialMediaService]);

  const setActiveAccount = useCallback((platformId: string, accountId: string) => {
    socialMediaService.setActiveAccount(platformId, accountId);
  }, [socialMediaService]);

  const getActiveAccount = useCallback((platformId: string) => {
    return socialMediaService.getActiveAccount(platformId);
  }, [socialMediaService]);
